...
export function TripCanvas({ trip, shared = false }: { trip: Trip; shared?: boolean }) {
  ...
  return <main className="aurora-page min-h-screen px-4 py-5 text-zinc-900 sm:px-8"><div className="mx-auto max-w-6xl">
    ...
    <section className="mt-4 grid gap-4 md:grid-cols-2"><article className="glass rounded-[22px] p-5"><p className="text-sm font-medium">Hidden gems</p><ul className="mt-4 space-y-3 text-sm text-zinc-600">{trip.experience.hidden_gems.map((item) => <li className="rounded-xl bg-white/45 p-3 transition hover:-translate-y-0.5" key={item}>✦ {item}<span className="float-right text-xs text-violet-600">Golden hour</span></li>)}</ul></article><article className="glass rounded-[22px] p-5"><p className="text-sm font-medium">Packing checklist</p><div className="mt-4 space-y-2">{trip.experience.packing.map((item) => <button onClick={()=>setChecked(c=>c.includes(item)?c.filter(x=>x!==item):[...c,item])} className="flex w-full items-center gap-3 rounded-xl bg-white/45 p-3 text-left text-sm text-zinc-600" key={item}><span className={`grid h-5 w-5 place-items-center rounded-full border ${checked.includes(item)?'border-violet-600 bg-violet-600 text-white':'border-zinc-300'}`}>{checked.includes(item)&&'✓'}</span>{item}</button>)}</div></article></section>
  </div></main>;
}
