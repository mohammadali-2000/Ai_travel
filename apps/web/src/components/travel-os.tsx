...
export function TravelOS() {
  ...
  function extractTrip() {
    const text = plannerPrompt.trim();
    const budget = text.match(/(?:₹|INR\s?)([\d,.]+(?:\s?(?:lakh|lac))?)/i);
    const people = text.match(/(\d+)\s*(?:people|travellers?|travelers?)/i);
    const destination = text.match(/(?:visit|to|in)\s+([A-Z][a-zA-Z\s]+?)(?:\s+(?:from|on|for|with|budget|,|\.)|$)/);
    const parsedBudget = budget ? budget[1].toLowerCase().includes('lakh') || budget[1].toLowerCase().includes('lac') ? String(Number(budget[1].replace(/[^\d.]/g, '')) * 100000) : budget[1].replace(/\D/g, '') : form.budget;
    setForm((current) => ({ ...current, destination: destination?.[1]?.trim() || current.destination, budget: parsedBudget || current.budget, currency: budget ? 'INR' : current.currency, travelers: people?.[1] || current.travelers, intent: text }));
    setExtracted(true);
  }
  ...
  return <main className="aurora-page relative min-h-screen overflow-hidden px-4 py-4 sm:px-7"><div className="aurora-blob one"/><div className="aurora-blob two"/><div className="relative mx-auto max-w-6xl"><header className="glass flex h-16 items-center justify-between rounded-2xl px-5"><div className="flex items-center gap-2.5"><span className="grid h-8 w-8 place-items-center rounded-xl bg-gradient-to-br from-violet-300 to-indigo-500 font-bold text-zinc-950">R</span><span className="text-[15px] font-medium">RoamVerse</span></div><span className="text-xs text-zinc-500">AI travel operating system</span></header>
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="grid gap-5 py-10 lg:grid-cols-[1.1fr_.9fr]"><section><p className="text-xs font-medium uppercase tracking-[.22em] text-violet-300">Create a Journey Drop</p><h1 className="mt-4 max-w-xl text-4xl font-medium tracking-[-.055em] sm:text-6xl">A trip worth remembering starts with a feeling.</h1><p className="mt-5 max-w-lg text-lg leading-7 text-zinc-400">Give your AI travel team a destination, time, and budget. They’ll create a complete experience—not a generic itinerary.</p>
      <form onSubmit={createTrip} className="glass mt-8 rounded-[22px] p-5 sm:p-6"><label className="block text-sm font-medium text-zinc-600">Describe your dream trip<textarea value={plannerPrompt} onChange={(e)=>setPlannerPrompt(e.target.value)} onBlur={extractTrip} className="input-glass mt-3 h-36 resize-none p-4 text-base" placeholder="I want to visit Japan from 1 Aug to 10 Aug. Budget around ₹1 lakh. I love anime, photography and local food." /></label><div className="mt-3 flex flex-wrap gap-2">{['🏖 Weekend Escape','🗻 Mountain Adventure','🍜 Food Tour','🏙 City Explorer','🏕 Backpacking','💕 Honeymoon'].map((idea)=><button type="button" onClick={()=>{setPlannerPrompt(idea+' in Japan with great food and photography'); setTimeout(extractTrip,0)}} className="rounded-full bg-white/65 px-3 py-2 text-xs text-violet-700 transition hover:-translate-y-0.5" key={idea}>{idea}</button>)}</div>{extracted&&<div className="mt-5 flex flex-wrap gap-2">{[['📍',form.destination||'Destination needed'],['💰',`${form.currency} ${form.budget}`],['👤',`${form.travelers} traveler(s)`]].map(([icon,value])=><span className="rounded-xl bg-violet-100 px-3 py-2 text-sm text-violet-800" key={value}>{icon} {value}</span>)}</div>}<div className="mt-5 grid gap-4 sm:grid-cols-2"><label className="text-sm text-zinc-600">Start date<input required type="date" value={form.start_date} onChange={(e)=>setForm({...form,start_date:e.target.value})} className={field}/></label><label className="text-sm text-zinc-600">End date<input required type="date" value={form.end_date} onChange={(e)=>setForm({...form,end_date:e.target.value})} className={field}/></label></div><button type="button" onClick={extractTrip} className="mt-4 text-sm text-violet-700">Refresh AI details ✦</button><button disabled={loading || !form.destination} className="mt-5 w-full rounded-xl bg-violet-600 px-4 py-3 text-sm font-medium text-white transition hover:scale-[1.01] disabled:opacity-50">{loading ? 'Your team is building the journey…' : 'Create my travel experience →'}</button>{error && <p role="alert" className="mt-3 text-sm text-rose-500">{error}</p>}</form></section>
